...
def studentlogging():
  ...
  if sessionType() == 4 or sessionType() == 5:
    ...
    cur.execute("SELECT * FROM applied_grad WHERE student_id = %s", (session['user_id'], ))
    ...
    return render_template("student.html", title = 'Student Logged In', data = data, suspended = suspended, 
                           registration = registration, semesters = semesters, cur_sem = cur_sem, x = x)
...
def assignadvisor():
  if sessionType() == 0 or sessionType() == 3:
    cur = db.cursor(dictionary = True)
